...
const ProductGridCard = ({
  ...
  const wishlistNotify = () =>
    toast.success("Item is been added to wishlist successfully", {
      position: "top-center",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  const errorNotify = () => {
    toast.error("somthing went wrong , unable to proceed your request !", {
      position: "top-center",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  const handleCart = async () => {
    try {
      const action = await dispatch(
        addToCartAsync({ product: id, user: userid })
      );
      if (!action.payload.Error === false) {
        errorNotify();
      } else {
  ...
  return (
    <>
      <div className="col-xl-4 col-md-6 col-sm-6 infinite-item w-full">
        <div className="tp-product-item-2 mb-10" id={id}>
          <div className="tp-product-thumb-2 p-relative z-index-1 fix w-img">
            <Link to={`${Path.PRODUCT_DETAILS}/${id}`} key={id}>
            ...
            <div className="tp-product-action-2 tp-product-action-blackStyle">
              <div className="tp-product-action-item-2 d-flex flex-column">
                ...
                <button
                  type="button"
                  className="tp-product-action-btn-2 tp-product-add-to-wishlist-btn z-50"
